...
class NewsViewModel(
    ...
    init {
        getBreakingNews(LANGUAGE_FOR_BREAKING_NEWS)
    }
    ...
    fun saveArticle(article: Article) = viewModelScope.launch {
        if (checkingArticleForUniqueness(article)) {
            newsRepository.upsert(article)
        }
    }

    fun saveArticleLive(article: Article) = viewModelScope.launch {
        newsRepository.upsert(article)
    }

    private suspend fun checkingArticleForUniqueness(article: Article): Boolean {
    ...
    fun getSavedNewsLive() = newsRepository.getSavedNewsLive()
